<div align="center">
  <h1>表情包素材库</h1>
  <p>给聊天和 AI Agent 准备的一份本地表情包与贴纸素材库。<br>原图、动图、描述与校验清单一起保存，换台电脑也能继续用。</p>
  <p><strong>2594 张表情包原图 · 1822 张 GIF · 全部附带描述 · 下载后可离线使用</strong></p>
  <p>
    <a href="#开始使用">开始使用</a> ·
...
这是一个可下载、可检索、可迁移的表情包素材集合。图片保存在 `emoticons/`，根目录的 `manifest.json` 记录每张图的相对路径、格式、大小、内容哈希和简短描述。公开版本已筛除不适合公开使用的图像和文案；私人收藏仍可由本地工具单独管理。

素材可由 `sticker-cli` 按需安装：默认使用 120 张精选表情包，需要完整内容时再显式选择全量包。下载后可以离线搜索、预览和迁移，不需要绑定微信或 MCP。
...
当前快照的图片内容约 **0.97 GiB**，完整克隆还需要 Git 数据的额外空间。`--depth 1` 只获取最新历史，仍会下载全部图片。也可以在仓库页面选择 **Code → Download ZIP**，解压后按同样的目录结构使用；ZIP 副本不能用 `git pull` 更新。
...
| Agent 使用 | 按 `AGENTS.md` 搜索、预览并展示表情包，无需运行专门的服务 |

当前快照包含 **1822 张 GIF、470 张 PNG、287 张 JPG 和 15 张 WebP**，共 **2594 张**。数量与描述以 [manifest.json](./manifest.json) 为准。
...
分包入口为 [packs.json](./packs.json)，其中 `all` 使用根目录清单，`curated` 使用 120 张精选清单；两者共享原图。包目录记录名称、描述、数量、总字节数及清单原始字节的 SHA-256 修订。完整约束与维护命令见[素材包格式](./docs/pack-format.md)。精选清单已逐张复核画面与描述，复核记录见[精选复核记录](./docs/curated-review.md)。
